...
def make_env(env_id, seed=1, render = None):
    def thunk():
        env = gym.make(env_id, render_mode=render)#continuous=True
...
class TD3():
    ...
    def __init__(self, args, env, writer=None):
        ...
        self.qf2 = QNetwork(env).to(args.device)
        self.qf1_target = QNetwork(env).to(args.device)
        self.qf2_target = QNetwork(env).to(args.device)
        ...
        self.qf2_target.double()

        self.q_optimizer1 = optim.Adam(list(self.qf1.parameters()), lr=args.learning_rate)
    ...
    def select_action(self, net, obs, noise_mul):
        ...
        noise = torch.normal(0, net.action_scale * noise_mul )
        actions += noise
    ...
    def update(self):
        for _ in range(20):
            ...
            with torch.no_grad():
                ...
                next_state_actions = torch.Tensor(self.select_action(self.target_actor,data.next_observations, self.noise_mul/2)).to(self.args.device)
                qf1_next_target = self.qf1_target(data.next_observations, next_state_actions)
                qf2_next_target = self.qf2_target(data.next_observations, next_state_actions)
                next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * self.args.gamma * (torch.min(qf1_next_target,qf2_next_target)).view(-1)
            ...
            self.q_update +=1
            if self.q_update % self.args.actor_delay == 0:
